...
### User Story 5 - Avatar Dashboard 與 Gmail 隱私說明 (Priority: P2)

使用者開啟 PWA 可看 **Avatar Dashboard**（大圖／動畫 + 當前狀態 + 未讀數）及 **Gmail 隱私說明**（明確告知只同步 INBOX 未讀總數，不讀取未讀信件內容或摘要）。

**Acceptance Scenarios**:

1. **Given** 已綁定且有未讀，**When** 進入 Dashboard，**Then** 顯示對應 Avatar 與未讀總數。
2. **Given** Gmail 隱私說明頁，**When** 使用者查看，**Then** 頁面說明 App 不讀取未讀信件清單、主旨、寄件者、摘要或正文。
...
- **FR-050**: MUST 提供 **Avatar Dashboard**、**模組選擇頁**、**Gmail 隱私說明頁**、**設定頁**（含**狀態門檻編輯器**與**模組選擇**子區）。
